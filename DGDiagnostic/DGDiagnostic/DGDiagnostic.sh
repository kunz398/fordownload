...
#  Name   : <script/ name>                                         #
#  Ver    : 1.0                                                    #
#  Depend : srdc_DGPrimary_diag.sql  and srdc_DGPhyStby_diag.sql   #
#  Date   : <date>                                                 #
####################################################################
path=`pwd`
...
if [ ! -f srdc_DGPrimary_diag.sql ]; then
   ...
   echo "    File srdc_DGPrimary_diag.sql doesn't exists in current directory ie \"$path\".  Script is exiting now.
    Please make sure file "Primary_diagnostic.zip" from document 2219763.1 is downloaded / copied to current directory ,\"$path\",
...
case `uname -s` in
        'SunOS')
          ...
          alertlog=`ls -lrt $dp/alert*.log|grep -i $SID|tail -1|awk -F"_" '{print "alert_"$NF}'|sed s/.log//`
          cp $dp/$alertlog.log  $path/$alertlog\_$dr.log
          
          brokerlog=`ls -lrt $dp/drc* 2>/dev/null |grep -i $SID|tail -1|awk -F"/" '{print $NF}'|sed s/.log//  2>/dev/null` 
        ...
        *)
          ...
          alertlog=`ls -lrt $dp/alert*.log|grep -i $SID|tail -1|awk -F"alert" '{print "alert"$2}'|sed s/.log//` 
          cp $dp/$alertlog.log  $path/$alertlog\_$dr.log
          
          brokerlog=`ls -lrt $dp/drc* 2>/dev/null |grep -i $SID|tail -1|awk -F"drc" '{print $2}'|sed s/.log//  2>/dev/null` 
          cp $dp/drc$brokerlog.log  $path/srdc_broker_log_drc_$brokerlog\_$dr.log                              2>/dev/null
		;;  
esac
...
echo " "                                                                                                                  >> listener_details\_$dr.log
     ...
     do
      ...
      echo "---"                                                                                                           >> listener_details\_$dr.log
      echo "  "                                                                                                            >> listener_details\_$dr.log
      `echo  $t|awk '{print $1}'|awk -F"bin" '{print $1}'`/bin/lsnrctl status `echo $t|awk '{print $2}'`                   >> listener_details\_$dr.log
     done
...
case `uname -s` in
        'SunOS')
		  export PRN_TRACE=`/usr/xpg4/bin/awk -v A=9 '/Slave exiting with ORA-600 exception|ERROR: ORA-00752/{f=A+1} f{print;f--}' $dp/$alertlog.log|grep "_pr[0-9a-z][0-9a-z]_"|tail -1|awk -F"file" '{print $2}'|awk -F: '{print $1}'`
		  if [ ! -z $PRN_TRACE ] 
           then  
echo "....Copying the PRnn trace......"				   
        ...
        ;;
        'HP-UX')
		  export PRN_TRACE=`awk -v A=9 '/Slave exiting with ORA-600 exception|ERROR: ORA-00752/{f=A+1} f{print;f--}' $dp/$alertlog.log|grep "_pr[0-9a-z][0-9a-z]_"|tail -1|awk -F"file" '{print $2}'|awk -F: '{print $1}'`
		  if [ ! -z $PRN_TRACE ] 
           then
echo "....Copying the PRnn trace......"		
		   `cp $PRN_TRACE $path/ 2>/dev/null`
